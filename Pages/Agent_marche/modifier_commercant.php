...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nom_complet)) {
        $error = 'Veuillez saisir le nom complet.';
    } else {
        try {
            $commercant_obj = new Commercant();
            $commercant_obj->update($commercant_id, $nom_complet, $telephone, $email, $password);
            $success = 'Commerçant modifié avec succès !';
        } catch (Exception $e) {
...
<head>
    ...
    <title>Modifier le commerçant</title>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-primary">
                    <i class="fas fa-edit text-accent mr-2"></i>Modifier le commerçant
                </h1>
            ...
            <?php else: ?>
                <form method="POST" class="space-y-4">
                    <div>
                        ...
                        <input type="text" value="<?= htmlspecialchars($commercant['matricule']) ?>" disabled
                    ...
                    <div>
                        ...
                        <input type="text" name="nom_complet" required
                               value="<?= htmlspecialchars($commercant['nom_complet']) ?>"
                    ...
                    <div>
                        ...
                        <input type="tel" name="telephone"
                               value="<?= htmlspecialchars($commercant['telephone'] ?? '') ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-accent focus:outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email"
                               value="<?= htmlspecialchars($commercant['email'] ?? '') ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-accent focus:outline-none">
                    </div>
