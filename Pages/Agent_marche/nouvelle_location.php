...
$etalages = $etalage->getDisponibles();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$id_etalage || !$id_commercant || !$montant) {
        $error = 'Veuillez remplir tous les champs obligatoires.';
    } else {
        try {
            $location = new Location();
            $location->create($id_etalage, $id_commercant, $montant, $duree);
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-bold text-primary">
                    <i class="fas fa-plus-circle text-accent mr-2"></i>Nouvelle location
                </h1>
                ...
                <form method="POST" class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Étalage <span class="text-red-500">*</span>
                        </label>
                        <select name="id_etalage" required 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:border-accent focus:outline-none">
                            <option value="">Sélectionner un étalage disponible</option>
                    ...
                    <button type="submit" class="w-full btn-accent py-2 rounded-lg font-semibold">
                        <i class="fas fa-save mr-2"></i> Créer la location
                    </button>
                </form>
            <?php endif; ?>
        </div>
